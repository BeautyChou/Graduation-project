...
type Direction struct {
	ID            int `json:"id" gorm:"primary_key;type:int(11)"`
	CreatedAt     time.Time
	UpdatedAt     time.Time
	DeletedAt     gorm.DeletedAt `sql:"index"`
	Faculty       Faculty        `gorm:"ForeignKey:FacultyID;"`                      // 方向所属学院
...
type Student struct {
	...
	Direction    Direction `gorm:"ForeignKey:DirectionID;"`                                       // 学生方向外键
	DirectionID  int       `json:"direction_id" gorm:"default:0;index:direction_id;type:int(11)"` // 学生所选方向
}
...
type DirectionForSelect struct {
	ID            int    `json:"value" gorm:"primary_key;type:int(11)"`
	FacultyID     int    `json:"faculty_id" gorm:"primary_key;type:int(11)"` // 学院ID
	DirectionName string `json:"name" gorm:"type:varchar(60)"`               // 方向名称
}
